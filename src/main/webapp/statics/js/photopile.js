(function ($,document) {
    ...
    var thumbOverlap      = 60;         // overlap amount (px)
	...
	var serial = 1;
	var photoPileTimer, loadPhotoPileTimer;
...
//	        });
//	        var photopiles = $('.photopile').find('li').get();
//	        if (photopiles.length != 0) {
//	        	photo.pickup($($('.photopile').find('li').get().random()));
//	        }
	        photopile.listPhotoPile();
	    	photopile.startPhotoPile();
		},
		startPhotoPile: function () {
			loadPhotoPileTimer = setInterval(photopile.loadPhotoPile,2000);
			photoPileTimer = setInterval(function(){
				photo.pickup($($('.photopile').find('li').get().random()))
			},3000);
		},
		stopPhotoPile: function () {
			clearInterval(loadPhotoPileTimer);
			clearInterval(photoPileTimer);
			photo.putDown();
		},
		loadPhotoPile: function () { //摇一摇开始
			$.ajax({
				url:'../activity-screen/loadPhotoPile',
				type: 'GET',
				data: {
				...
				success: function (response) {
					...
					if ($('.photopile').find('li').length >= 40) {
						var randPhoto = $($('.photopile').find('li').get().random());
						randPhoto.find('a').attr('imgSrc', response.data.imgSrc);
						randPhoto.find('img').attr('src', response.data.imgSrc);
						return;
					}
					serial = serial + 1;
					var tailLi = $('<li>'
						+ '<a imgSrc="' + response.data.imgSrc + '">'
						+ '<img src="' + response.data.imgSrc + '" alt="" style="width:133px;heigh:100px;" />'
						+ '</a>'
						+ '</li>');
					$('.photopile').append(tailLi);
					thumb.init($(tailLi));
				}
			});
		},
		listPhotoPile: function () { //摇一摇开始
			$.ajax({
				url:'../activity-screen/listPhotoPile',
				type: 'GET',
				data: {
					activityMid: activityMid
				},
				success: function (response) {
					...
					$.each(response.items, function(i, item){   
						var tailLi = $('<li>'
								+ '<a imgSrc="' + item.imgSrc + '">'
								+ '<img src="' + item.imgSrc + '" alt="" style="width:133px;heigh:100px;" />'
								+ '</a>'
								+ '</li>');
						$('.photopile').append(tailLi);
						thumb.init($(tailLi));
	...
	var thumb = {
	        ...
	        getActiveImgSrc   : function() { return $('li.' + this.active).children().first().attr('imgSrc'); },
...
//            },3000);
        ...
        loadImage : function ( src, callback ) {
            ...
            img.onload = function() {               // Restore visibility in callback
            	if (this.width > 500) {
            		self.fullSizeWidth = 500;
            	} else {
            		self.fullSizeWidth = this.width;
            	}
            	
            	if (this.height > 500) {
            		self.fullSizeHeight = 500;
            	} else {
...
//        },
	...
	photopile.init();
	window.photopile = photopile;
	window.photopileLoaded = true;
})(jQuery,document);